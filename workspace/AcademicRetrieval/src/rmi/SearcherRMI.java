...
public class SearcherRMI extends UnicastRemoteObject implements
		SearcherRMIInterface {
	...
	public SearcherRMI() throws CorruptIndexException, IOException,
			ClassNotFoundException {
		searcher = new TextSearcher();
		associationModel = (TopicAssociationModel) SerializeUtils
				.deSerialize(TopicAssociationModel.storagePath);
	}
	...
	public AssociateResult[] getAssociatePapers(int papertmId)
			throws IOException {
		...
		for (int i = 0; i < 5; ++i) {
			...
			sb.append("Topic : ");
			for (int k = 0; k < associationModel.topics[topicid].topRelatedWordIds
					.size(); ++k) {
				String word = searcher.cache
						.getWordByTmIndex(associationModel.topics[topicid].topRelatedWordIds
								.get(k)).Word;
				if (k < 5) {
					sb.append(word + "; ");
				}
		...
		System.out.println("Querying cache runtime (ms):"
	...
	@Override
	public AssociateResult[] getAssociateAuthors(int authorId)
			throws IOException {
		long before = System.currentTimeMillis();
		AssociateResult[] re = new AssociateResult[5];
		for (int i = 0; i < 5; ++i) {
			...
			int topicid = associationModel.authorRelatedTopic[authorId][i];
			re[i].topic = associationModel.topics[topicid];
			StringBuffer sb = new StringBuffer();
			sb.append("Topic : ");
			for (int k = 0; k < associationModel.topics[topicid].topRelatedWordIds
