...
function showRelatedStampDialog(o,inner)
{
	...
	var ob0 = document.getElementById("rs_glx0");
		ob0.length = 0;
	var ob1 = document.getElementById("rs_glx1");
		ob1.length = 0;
	var ob2 = document.getElementById("rs_glx2");
		ob2.length = 0;
	var ob3 = document.getElementById("rs_glx3");
		ob3.length = 0;
	for(i = 0;i<tm0.length;i++)
	{
		ob0.appendChild(createOption(tm1[i],tm0[i]));
		ob1.appendChild(createOption(tm1[i],tm0[i]));
		ob2.appendChild(createOption(tm1[i],tm0[i]));
		ob3.appendChild(createOption(tm1[i],tm0[i]));
	}
	
	document.getElementById("avitShowName").innerHTML = "视频"+inner;
...
{
	var paths = new Array(
		"/AUDRweb/mainimage.html?rs=video",
		"/AUDRweb/TextSearch?rs=video",
		"/AUDRweb/AudioSearch?rs=video",
		"/AUDRweb/VideoSearch?rs=video"
		);
...
function showDlg()
{
	var objDeck = document.getElementById("deck");
        ...
        objDeck.style.MozOpacity=40/100;
       	...
       	document.getElementById("rs_div_mdplay").style.display = "none";
...
function hiddenDlg()
{
	...
	document.getElementById("rs_div_mdplay").style.display = "";
