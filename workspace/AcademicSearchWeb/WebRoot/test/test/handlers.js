...
function uploadSuccess(file, serverData) {
	var isSuccess = (serverData.indexOf("0")==0?false:true);
	//alert(serverData);
	try {
		if(isSuccess){
			allfiles.push(serverData);
			...
			//jjun添加代码处理页面
			//alert(flag+":"+this.getStats().files_queued+":"+allfiles);
			if(this.getStats().files_queued=='0'&&flag==0)
			{
			...
			//document.getElementById("file_addform_div_file").innerHTML = allfiles;
		}else{
			flag++;
			...
			alert("上传失败：可能是"+file.name+"名称重复！！！");
			document.getElementById("file_addform_add_submit").disabled = true;
			document.getElementById("file_addform_add_file").value = 0;
		}
	} catch (ex) {
		this.debug(ex);
	}
